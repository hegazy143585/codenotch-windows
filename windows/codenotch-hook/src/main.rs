...
fn main() {
    ...
    // `--provider <id>` / `--provider=<id>` (or the CODENOTCH_PROVIDER env var) tags this event as
    // belonging to a non-Claude module, so any tool with a hook/notify mechanism reports its working
    // state through the exact same transport Claude Code uses. Absent → Claude, the original behaviour.
    let rest: Vec<String> = args.collect();
    let mut provider = std::env::var("CODENOTCH_PROVIDER").unwrap_or_default();
    let mut i = 0;
    while i < rest.len() {
        if let Some(v) = rest[i].strip_prefix("--provider=") {
    ...
    if send(port, &event, ppid, &provider, &body).is_ok() {
        return;
    }
...
fn send(port: u16, event: &str, ppid: u32, provider: &str, body: &str) -> std::io::Result<()> {
    ...
    let prov = if provider.is_empty() { String::new() } else { format!("&p={provider}") };
    let req = format!(
