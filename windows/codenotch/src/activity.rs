...
/// A pushed "busy"/"waiting" that is never followed by a clearing event (the tool crashed mid-run,
/// or its integration is buggy) must not pin the ring green forever. Matches the four-state engine's
/// RUNNING_STALE: a run with no event for 30 min is assumed dead.
const PUSH_TTL_MS: u64 = 30 * 60_000;
...
// ---------------- Generic event ingress (the same transport Claude Code's hooks use) ----------------
//
// The reverse-engineered probes below each reverse a single vendor's private local files, so each is
// one vendor update away from going dark, and a provider with no probe (Copilot, GLM, Grok, Ollama,
// opencode…) can never show a working state at all. The reliable path has always been the other one:
// Claude Code *tells* Codenotch what happened through codenotch-hook. This registry opens that path to
// every provider. Any tool that can run a command on an event — Codex's `notify`, Cursor's hooks, a
// one-line shell wrapper — calls `codenotch-hook <event> --provider <id>`; the event server routes it
// here; and the provider appears in the notch with full running/waiting state, no bespoke probe and no
// change to the notch UI. Pushed state is authoritative: while it is present for a provider, that
// provider's probe (if any) is skipped, so an accurate event always beats a guess.

struct Pushed {
    act: Activity,
    /// ms epoch after which this entry is dropped if no clearing/refresh event arrived (crash guard)
    expires: u64,
}
...
/// Ingest one pushed event. `state` accepts the same internal event names the Claude hook uses, so one
/// wiring table drives both: running/busy/start → busy; attention/waiting → waiting; anything else
/// (done/idle/stop/session_end) clears the row. Empty session collapses to the provider's single row.
pub fn push(provider: &str, session: &str, state: &str, name: &str, detail: &str) {
    if provider.is_empty() {
        return;
    }
    let now = now_ms();
    let key = format!("{provider}\u{0}{}", if session.is_empty() { provider } else { session });
    let mapped = match state {
        "running" | "busy" | "start" | "session_start" => Some("busy"),
        "attention" | "waiting" => Some("waiting"),
        _ => None,
    };
    ...
    match mapped {
        Some(st) => {
            ...
            let since = match map.get(&key) {
                Some(p) if p.act.state == st => p.act.since,
                _ => now,
            };
            map.insert(
                key,
                Pushed {
                    act: Activity {
                        ...
                        state: st.into(),
                        name: if name.is_empty() { default_name(provider) } else { name.into() },
...
/// Is a provider working right now, by the notch's own view (pushed or probed)? The usage pollers
/// use it to tighten their cadence while a run is in flight, whatever source reported the run.
pub fn provider_active(app: &AppHandle, provider: &str) -> bool {
    let st = app.state::<AppState>();
    let rows = st.activity.lock().unwrap();
    rows.iter().any(|a| a.provider == provider)
}

/// Non-expired pushed rows; drops anything past its TTL in passing.
fn pushed_activities() -> Vec<Activity> {
    let now = now_ms();
    let mut map = PUSHED.lock().unwrap();
    map.retain(|_, p| p.expires > now);
    map.values().map(|p| p.act.clone()).collect()
}
...
fn read_all(p: Presence, ctx: &mut Ctx) -> Vec<Activity> {
    // Pushed events first, and authoritative: a provider that reported through the generic ingress has
    // its probe skipped this tick (an accurate event beats a reverse-engineered guess). Providers with
    // no probe at all reach the notch purely through this path.
    let mut all = pushed_activities();
    let has: std::collections::HashSet<String> = all.iter().map(|a| a.provider.clone()).collect();
    if !has.contains("claude") {
        all.extend(claude_activity());
    }
    if p.cursor && !has.contains("cursor") {
        all.extend(cursor_activity(ctx));
    }
    if p.codex && !has.contains("codex") {
        all.extend(codex_activity(ctx));
    }
    if p.gemini && !has.contains("gemini") {
        all.extend(antigravity_activity());
    }
    // Deterministic order: PUSHED is a HashMap, so without this the vector's order would jitter between
    // ticks and the `found != last` change check would fire every 2 s — a broadcast storm.
    all.sort_by(|a, b| a.provider.cmp(&b.provider).then(b.since.cmp(&a.since)).then(a.name.cmp(&b.name)));
    all
}
...
mod tests {
    ...
    fn row(prov: &str) -> Option<Activity> {
        pushed_activities().into_iter().find(|a| a.provider == prov)
    }

    #[test]
    fn default_name_capitalizes() {
        assert_eq!(default_name("copilot"), "Copilot");
        assert_eq!(default_name(""), "");
    }

    #[test]
    fn running_shows_busy_then_a_clearing_event_removes_it() {
        // unique provider id so the shared PUSHED map does not collide with other tests
        push("t_copilot", "s1", "running", "", "");
        let a = row("t_copilot").expect("busy row present");
        assert_eq!(a.state, "busy");
        assert_eq!(a.name, "T_copilot");
        assert_eq!(a.detail, "Working");
        // a "done" (or stop/session_end/idle) clears the row
        push("t_copilot", "s1", "done", "", "");
        assert!(row("t_copilot").is_none());
    }

    #[test]
    fn waiting_maps_to_attention_and_keeps_explicit_labels() {
        push("t_glm", "s1", "attention", "Build", "approve shell?");
        let a = row("t_glm").expect("waiting row present");
        assert_eq!(a.state, "waiting");
        assert_eq!(a.name, "Build");
        assert_eq!(a.detail, "approve shell?");
        push("t_glm", "s1", "session_end", "", "");
        assert!(row("t_glm").is_none());
    }

    #[test]
    fn a_flip_from_busy_to_waiting_resets_since() {
        push("t_grok", "s1", "running", "", "");
        let busy_since = row("t_grok").unwrap().since;
        std::thread::sleep(std::time::Duration::from_millis(3));
        push("t_grok", "s1", "waiting", "", "");
        let a = row("t_grok").unwrap();
        assert_eq!(a.state, "waiting");
        assert!(a.since >= busy_since, "since should advance on a state flip");
        push("t_grok", "s1", "stop", "", "");
    }

    #[test]
    fn several_sessions_of_one_provider_are_separate_rows() {
        push("t_ollama", "a", "running", "", "");
        push("t_ollama", "b", "running", "", "");
        let n = pushed_activities().iter().filter(|x| x.provider == "t_ollama").count();
        assert_eq!(n, 2);
        push("t_ollama", "a", "done", "", "");
        push("t_ollama", "b", "done", "", "");
    }

    #[test]
    fn expired_rows_are_dropped() {
        push("t_stale", "s1", "running", "", "");
        // force expiry in the past
        PUSHED.lock().unwrap().get_mut("t_stale\u{0}s1").unwrap().expires = 1;
        assert!(row("t_stale").is_none());
    }
}
