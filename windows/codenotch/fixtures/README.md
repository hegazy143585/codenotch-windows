...
| `claude_oauth_usage*.json`, `codex_wham_usage.json`, `cursor_usage_summary_*.json`, `antigravity_bridge.json`, `gemini_cli_chat.jsonl`, `ollama_ps.json`, `ollama_cloud_usage.json`, `glm_quota_limit.json` (shapes from the macOS reader's recorded reply) | Hand-built from the reply shapes the parsers read. Replace with sanitized recordings when available |
